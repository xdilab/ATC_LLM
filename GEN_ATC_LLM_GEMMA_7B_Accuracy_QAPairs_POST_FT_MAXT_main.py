# GEMMA-7B Fine-tuning & QA Evaluation Pipeline
...
for path in pdf_paths:
    if content := extract_pdf_text(path):
        text_data.extend(content)
    else:
        print(f" Skipping unreadable PDF: {path}")
...
class PDFDataset(Dataset):
    def __init__(self, ids, masks, labels):
        self.data = [{"input_ids": ids[i], "attention_mask": masks[i], "labels": labels[i]} for i in range(len(ids))]
...
class NoOptimizerSaveTrainer(Trainer):
    def _save_optimizer_and_scheduler(self, output_dir):
        print(" Skipping optimizer/scheduler save to conserve space.")

trainer = NoOptimizerSaveTrainer(
    model=model,
    args=training_args,
...
trainer.train()
print(f" Fine-tuning complete in {(time.time() - start_time_train)/60:.2f} min")
...
def generate_answer(question, reference=None):
    max_tokens = int(len(reference.split()) * 1.3) if reference else 128
...
def calculate_perplexity(text):
    with torch.no_grad():
        ...
        return torch.exp(loss).item() if loss else np.nan

# Evaluation Loop
results_path = "./gemma_finetuned_eval_results_met_bert_MAXT.csv"
evaluation_start = time.time()
accuracy_results = []

for doc_name, qa_data in qa_pairs.items():
    print(f"\n Evaluating on: {doc_name}")
    manual_scores, cosine_scores, bleu_scores, rouge_scores, meteor_scores, bert_scores, edit_dists, perplexities = [], [], [], [], [], [], [], []

    for question, (reference, _) in zip(qa_data["questions"], qa_data["answers"]):
        prediction = generate_answer(question, reference)
        manual = 1 if reference.lower() in prediction.lower() else 0
        ...
        bert_p, bert_r, bert_f1 = bert_score([prediction], [reference], lang="en", verbose=False)
        edit_dist = editdistance.eval(reference.split(), prediction.split())
        ppl = calculate_perplexity(prediction)
...
with open(results_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["model_name", "document", "manual_accuracy", "cosine_similarity", "bleu_score", "rougeL_score",
                     "avg_edit_distance", "avg_perplexity", "meteor_score", "bert_score"])
    writer.writerows(accuracy_results)

# Done
total_eval_time = time.time() - evaluation_start
print(f"\n Results saved to `{results_path}`")
print(f" Eval Time: {total_eval_time / 60:.2f} min")
print(f" GPU Memory: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
print(f" CPU Memory: {psutil.virtual_memory().used / 1e9:.2f} GB")
