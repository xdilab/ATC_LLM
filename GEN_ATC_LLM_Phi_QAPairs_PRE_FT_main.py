...
tokenizer = AutoTokenizer.from_pretrained(fresh_model_path, use_fast=True)
...
def generate_answer(question, reference=None):
    prompt = f"Context: {reference}\n\nQuestion: {question}\nAnswer:" if reference else question
...
# === Evaluation ===
csv_file = "/home/smgreen1/PycharmProjects/GEN_ATC_LLM_Scripts/phi4_eval_results_FRESH_V2.csv"
results = []

print("\n📊 Evaluating fresh model on QA pairs...")
start_time = time.time()

for doc_name, qa_data in qa_pairs.items():
    manual, cosine, bleu, rouge, edit, perplex, berts, meteors = [], [], [], [], [], [], [], []

    for question, (reference, _) in zip(qa_data["questions"], qa_data["answers"]):
        prediction = generate_answer(question, reference)

        manual.append(1 if reference.lower() in prediction.lower() else 0)
        ref_embed = embedding_model.encode(reference)
        pred_embed = embedding_model.encode(prediction)
        cosine.append(cosine_similarity([ref_embed], [pred_embed])[0][0])
        bleu.append(sentence_bleu([reference.split()], prediction.split(), smoothing_function=smooth))
        rouge.append(scorer.score(reference, prediction)["rougeL"].fmeasure)
        edit.append(editdistance.eval(reference.split(), prediction.split()))
        perplex.append(calculate_perplexity(prediction))
        meteors.append(meteor_score([reference.split()], prediction.split()))

        P, R, F1 = bert_score([prediction], [reference], lang="en", verbose=False)
        berts.append(F1.item())

    results.append([
        model_name,
        doc_name,
        sum(manual) / len(manual),
        sum(cosine) / len(cosine),
        sum(bleu) / len(bleu),
        sum(rouge) / len(rouge),
        sum(edit) / len(edit),
        sum(perplex) / len(perplex),
        sum(berts) / len(berts),
        sum(meteors) / len(meteors)
    ])
...
with open(csv_file, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow([
        "model_name", "document", "manual_accuracy", "cosine_similarity", "bleu_score",
        "rougeL_score", "edit_distance", "perplexity", "bertscore", "meteor"
    ])
    writer.writerows(results)

print(f"\n✅ Fresh model evaluation results saved to: {csv_file}")
print(f"⏱️ Total Evaluation Time: {(time.time() - start_time) / 60:.2f} minutes")
print(f"GPU Memory Used: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
print(f"CPU Memory Used: {psutil.virtual_memory().used / 1e9:.2f} GB")
