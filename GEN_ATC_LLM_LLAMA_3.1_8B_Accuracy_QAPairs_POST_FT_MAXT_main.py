...
print("\n🧠 Pre-tokenization memory:")
print(f"GPU Allocated: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
print(f"GPU Reserved: {torch.cuda.memory_reserved() / 1e9:.2f} GB")
print(f"CPU Used: {psutil.virtual_memory().used / 1e9:.2f} GB")
...
for i in range(0, len(text_data), batch_size):
    ...
    encoded = tokenizer(batch, max_length=512, padding="max_length", truncation=True, return_tensors="pt").to(device)
...
class GrokDataset(Dataset):
    def __init__(self, ids, masks, labels):
        self.data = []
        for i in range(len(ids)):
            if ids[i].dim() > 0:  # <- Skip if tensor is not 1D
                self.data.append({
                    "input_ids": ids[i],
...
dataset = GrokDataset(input_ids, attention_masks, labels)
...
try:
    ...
    if "CUDA out of memory" in str(e):
        print("Try lowering batch size or enabling gradient checkpointing.")

print("\n🧠 Post-training memory:")
print(f"Peak GPU: {torch.cuda.max_memory_allocated() / 1e9:.2f} GB")
print(f"Current GPU: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
print(f"CPU Used: {psutil.virtual_memory().used / 1e9:.2f} GB")
...
def generate_answer(question, reference=None):
    if reference:
        # max_tokens = int(len(reference.split()) * 1.25)
        max_tokens = int(len(reference.split()) * 1.3)
        max_tokens = min(max_tokens, 192)

    else:
        max_tokens = 128  # fallback
    ...
    outputs = model.generate(
        ...
        length_penalty=0.7,
        early_stopping=True,
        no_repeat_ngram_size=3  # 💡 helps eliminate repetitive phrasing
    )
...
for doc_name, qa_data in qa_pairs.items():
    ...
    for question, (reference, _) in zip(qa_data["questions"], qa_data["answers"]):
        ...
        rouge_val = scorer.score(reference, prediction)["rougeL"].fmeasure
        ...
        print(f"✔️ Manual: {manual_acc} | Cosine: {cos_sim:.2f} | BLEU: {bleu:.2f} | ROUGE-L: {rouge_val:.2f}")
        print(f"🧮 Edit Dist: {edit_dist} | Perplexity: {perplexity:.2f} | BERT F1: {F1[0].item():.2f} | METEOR: {meteor:.2f}")
...
# 💾 Save to CSV
csv_file = "llama_metrics_finetuned_bertandmet_x1results.csv"
with open(csv_file, mode="w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow([
        "model_name", "document", "manual_accuracy", "cosine_similarity", "bleu_score", "rougeL_score",
        "avg_edit_distance", "avg_perplexity", "bert_score_f1", "meteor_score",
        "num_train_epochs", "per_device_train_batch_size", "gradient_accumulation_steps", "learning_rate", "fp16"
    ])
    for doc_name, metrics in accuracy_results.items():
        writer.writerow([
            model_name, doc_name,
            metrics["manual_accuracy"], metrics["cosine_similarity"], metrics["bleu_score"], metrics["rougeL_score"],
            metrics["avg_edit_distance"], metrics["avg_perplexity"],
            metrics["bert_score_f1"], metrics["meteor_score"],
...
print(f"⏱️ Total Runtime: {((time.time() - start_time) / 60):.2f} min")
