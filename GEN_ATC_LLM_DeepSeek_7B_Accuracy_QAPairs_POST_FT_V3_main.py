...
# Dataset Definition with Instruction Tuning
class InstructionTunedDataset(Dataset):
    def __init__(self, qa_pairs, tokenizer):
        self.data = []
        for doc, qa in qa_pairs.items():
            for question, (answer, _) in zip(qa["questions"], qa["answers"]):
                prompt = f"[INST] You are an FAA advisor. Answer the following question based on aviation knowledge:\n{question} [/INST]"
                encoded = tokenizer(prompt, max_length=512, padding="max_length", truncation=True, return_tensors="pt")
                label_encoded = tokenizer(answer, max_length=512, padding="max_length", truncation=True, return_tensors="pt")
...
dataset = InstructionTunedDataset(input_ids, attention_masks, labels)
...
for doc_name, qa_data in qa_pairs.items():
    ...
    for question, (reference, _) in zip(qa_data["questions"], qa_data["answers"]):
        ...
        rouge_score_val = scorer.score(reference, prediction)
        edit_dist = editdistance.eval(reference.split(), prediction.split())
        perplexity = calculate_perplexity(prediction)

        scores.append((manual_acc, cos_sim, bleu, rouge_score_val['rougeL'].fmeasure, edit_dist, perplexity))

    accuracy_results[doc_name] = scores

# Save Results to CSV
csv_file = "/home/smgreen1/PycharmProjects/GEN_ATC_LLM_Scripts/deepseek_metrics.csv"
with open(csv_file, mode="w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["document", "manual_accuracy", "cosine_similarity", "bleu_score", "rougeL_score", "edit_distance", "perplexity"])
    for doc_name, metrics in accuracy_results.items():
        writer.writerow([doc_name] + list(metrics[0]))
